...
- [x] イベントバスの購読者としてvoiceハンドラを追加。WS接続中のクライアントに通知を配送(セッション完了イベントもバスに追加。Pushoverには流さない)
- [x] イベント種別ごとの読み上げポリシー設定(`speak` / `chime` / `silent`)。CLI: `alir voice notify set question=speak session_done=chime`。DBから都度読むためserveの再起動なしで効く
- [x] 質問イベントは本文が長いので、TTS前にLLM(`claude -p`)で口頭向けに要約。失敗時は機械的な要約にフォールバック
- [x] クライアント側: 通知読み上げの前にチャイム音を鳴らす(WebAudioの2音。音源ファイル不要)
- [x] 未接続時のフォールバック: Pushoverはバスの独立した購読者なので、WSクライアントの有無によらず従来通り届く
...
- [x] 通話モードのオン/オフをスマホ画面のトグルで切り替え(常時リスニングが不要な時間帯用)。ミュートトグルとして実装: マイク送信とサーバ側の区間切り出しだけを止め、接続と通知(チャイム・読み上げ)は生かす
- [ ] 必要ならウェイクワード(openWakeWord)。通話タブ+ミュートで足りているため見送り。欲しくなったら再検討
- [x] Whisperのhallucination対策: 幻聴の定型句(「ご視聴ありがとうございました」等)を部分一致で破棄(voice.is_hallucination)
- [x] 認証: WS接続時のトークン検証。`alir voice token set` で有効化し、`/voice?token=...` で開くとWSへ引き継ぐ。未設定なら従来通り検証しない
- [x] 複数クライアント対応。配送ルールは「接続中の全クライアントに通知を配送、意図解釈のセッションと発話処理は接続ごとに独立」(Phase 2/3 の実装がこの形)
